...
def get_args():
    parser = argparse.ArgumentParser(description="Convert .nii.gz to .npz (Lite v2 - Improved)")
...
def compute_sdf_improved(query_points, mesh_vertices, mesh_faces, batch_size=5000):
    """
    Improved SDF computation with better inside/outside detection
    核心改进：使用更激进的阈值和多重判断
    """
    ...
    mesh_std = mesh_vertices.std(axis=0).mean()  # Average std across dimensions
    ...
    for i in range(0, n_points, batch_size):
        ...
        # Multiple heuristics for better inside/outside detection
        
        # Heuristic 1: Distance to center
        to_center = batch_points - mesh_center
        dist_to_center = np.linalg.norm(to_center, axis=1)
        
        # Heuristic 2: Adjusted threshold based on mesh size
        # 使用更激进的阈值：0.8 而不是 1.5
        # 这会让更多点被判定为"内部"
        threshold = 0.8
        inside_by_center = dist_to_center < dists * threshold
        
        # Heuristic 3: Distance relative to mesh spread
        # 如果点距离质心的距离小于 mesh 的标准差，更可能在内部
        inside_by_spread = dist_to_center < mesh_std * 2.5
        
        # Combined decision: 两个启发式都认为是内部才算内部
        # 这样可以减少假阳性（错误判断为外部）
        is_inside = inside_by_center | inside_by_spread
...
def process_file(file_path, args):
    try:
        ...
        # Compute SDF with improved algorithm
        print(f"    Computing vol_sdf ({len(vol_points)} points)...")
        vol_sdf = compute_sdf_improved(vol_points, all_verts, all_faces, batch_size=args.batch_size)
        ...
        near_sdf = compute_sdf_improved(near_points, all_verts, all_faces, batch_size=args.batch_size)
        
        # Quality check
        vol_pos_ratio = (vol_sdf > 0).sum() / vol_sdf.size
        near_pos_ratio = (near_sdf > 0).sum() / near_sdf.size
        print(f"    Quality: vol_pos={vol_pos_ratio*100:.1f}%, near_pos={near_pos_ratio*100:.1f}%")
...
def main():
    ...
    print(f"Improved algorithm: threshold=0.8, dual heuristics")
    print("="*70)
