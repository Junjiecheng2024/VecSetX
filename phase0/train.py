...
# Modified for Phase 0: Single-class (Myocardium) training
...
def get_args_parser():
    parser = argparse.ArgumentParser('VecSet Phase 0 - Myocardium Single Class', add_help=False)
    ...
    parser.add_argument('--classes', type=int, nargs='+', default=[1],
                        help='Class IDs to train on (default: [1] for Myocardium)')
...
def main(args):
    ...
    print('=' * 60)
    print('VecSet Phase 0: Single Class Training (Myocardium)')
    print('=' * 60)
    ...
    # Create model
    # Note: input_dim=3 (no one-hot), output_dim=1 (single SDF)
    ...
    if args.use_wandb and global_rank == 0:
        ...
        wandb.init(
            ...
            name=args.wandb_run_name or f"phase0_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}",
            config=vars(args),
        )
