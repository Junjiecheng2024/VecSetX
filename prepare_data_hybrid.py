...
# mesh_to_sdf for near surface (accurate)
try:
    import mesh_to_sdf
...
def get_args():
    parser = argparse.ArgumentParser(description="Hybrid: V2 for vol_sdf, mesh_to_sdf for near_sdf")
...
def compute_sdf_v2_heuristic(query_points, mesh_vertices, batch_size=5000):
    """
    V2 heuristic (threshold=0.8) - Good for volume points
    """
...
def compute_sdf_mesh_to_sdf(query_points, mesh):
    """
    mesh_to_sdf - Accurate for near surface points
    """
...
def process_file(file_path, args):
    try:
        ...
        # HYBRID APPROACH:
        # Use V2 heuristic for volume points (better distribution)
        print(f"    Computing vol_sdf with V2 heuristic...")
        vol_sdf = compute_sdf_v2_heuristic(vol_points, all_verts, batch_size=args.batch_size)
        ...
        # Use mesh_to_sdf for near surface points (more accurate)
...
def main():
    ...
    print(f"  - vol_sdf: V2 heuristic (threshold=0.8, dual criteria)")
    print(f"  - near_sdf: mesh_to_sdf (professional accuracy)")
    print("="*70)
