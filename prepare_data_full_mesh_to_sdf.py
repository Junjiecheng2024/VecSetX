...
# mesh_to_sdf - professional SDF library
try:
    import mesh_to_sdf
...
def get_args():
    parser = argparse.ArgumentParser(description="Full mesh_to_sdf: Professional quality for both vol and near")
...
def compute_sdf_professional(query_points, mesh):
    """
    Professional SDF using mesh_to_sdf library
    This gives the most accurate results
    """
    if not MESH_TO_SDF_AVAILABLE:
        raise ImportError("mesh_to_sdf required. Install: pip install mesh-to-sdf")
    
    # Use mesh_to_sdf with optimal settings
    sdf_values = mesh_to_sdf.mesh_to_sdf(
        mesh, 
        query_points,
        surface_point_method='sample',  # Works on headless servers
        sign_method='normal',            # Most accurate
        scan_count=100,                  # Good quality
        scan_resolution=400              # High resolution
    )
...
def process_file(file_path, args):
    try:
        ...
        # PROFESSIONAL APPROACH: Use mesh_to_sdf for BOTH
        print(f"    Computing vol_sdf with mesh_to_sdf (professional)...")
        vol_sdf = compute_sdf_professional(vol_points, combined_mesh)
        ...
        print(f"    Computing near_sdf with mesh_to_sdf (professional)...")
        near_sdf = compute_sdf_professional(near_points, combined_mesh)
        ...
        vol_mean = vol_sdf.mean()
        near_mean = near_sdf.mean()
        
        print(f"    Quality: vol[+{vol_pos_ratio*100:.1f}% -{vol_neg_ratio*100:.1f}% mean={vol_mean:.3f}] near[+{near_pos_ratio*100:.1f}% -{near_neg_ratio*100:.1f}% mean={near_mean:.3f}]")
...
def main():
    ...
    if not MESH_TO_SDF_AVAILABLE:
        print("\n" + "="*70)
        print("ERROR: mesh_to_sdf library is required!")
        print("Install with: pip install mesh-to-sdf")
        print("="*70)
        return
    ...
    print(f"=" * 70)
    print(f"PROFESSIONAL QUALITY - mesh_to_sdf for BOTH vol and near")
    print(f"Expected: ~50/50 distribution for both vol_sdf and near_sdf")
    print(f"Estimated time: 2-5 min per file")
    print(f"=" * 70)
    ...
    print(f"Professional quality SDF data generation complete!")
    print(f"="*70)
