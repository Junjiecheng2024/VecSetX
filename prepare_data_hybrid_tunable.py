...
def get_args():
    parser = argparse.ArgumentParser(description="Hybrid with tunable threshold for perfect 50/50")
    ...
    parser.add_argument("--threshold", type=float, default=0.6, 
                       help="Threshold for inside/outside detection (lower=more inside). Default 0.6 targets ~50/50")
    return parser.parse_args()

def compute_sdf_tunable(query_points, mesh_vertices, threshold=0.6, batch_size=5000):
    """
    Tunable heuristic - adjust threshold to control positive/negative ratio
    threshold=0.8 → ~73% positive
    threshold=0.6 → ~50% positive (target)
    threshold=0.5 → ~25% positive
    """
...
def compute_sdf_mesh_to_sdf(query_points, mesh):
    """mesh_to_sdf for accurate near surface"""
...
def process_file(file_path, args):
    try:
        ...
        # Use tunable heuristic for vol_sdf
        print(f"    Computing vol_sdf (threshold={args.threshold})...")
        vol_sdf = compute_sdf_tunable(vol_points, all_verts, threshold=args.threshold, batch_size=args.batch_size)
...
def main():
    ...
    print(f"HYBRID TUNABLE:")
    print(f"  - vol_sdf: Heuristic (threshold={args.threshold})")
    print(f"  - near_sdf: mesh_to_sdf (professional)")
    print("="*70)
