...
#SBATCH --job-name=vecset_phase1_org
#SBATCH --nodes=1
...
torchrun --nproc_per_node=4 --master_port=29500 phase1_reconstruction/train.py \
    ...
    --input_dim 13 \
    --epochs 400 \
    --data_path /scratch/project_2016517/junjie/dataset/original_npz \
    --output_dir output/ae/phase1_production_org \
    --log_dir output/ae/phase1_production_org \
    --blr 2e-4 \
    --warmup_epochs 20 \
    --wandb

# KEY CHANGE LOG (Org - Verified Stable):
# 1. blr: 2e-4 (Standard conservative rate)
# 2. Weights: loss_near=0.5, loss_surface=0.5, loss_cls=5.0 (Hardcoded in engine_ae.py)
# 3. Output: org directory
